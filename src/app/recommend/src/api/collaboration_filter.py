...
class CollaborationFilter():
    ...
    def infer(self, current_user=226):
        # try it out
        similar_user_indices = self.similar_users(current_user, self.rating_matrix)
        print(f"{similar_user_indices=}")

        # try it out
        recommended = self.recommend_item(226, similar_user_indices, self.rating_matrix)
        return recommended
    ...
    def get_filter_list(self, product_name) -> list[str]:
        ...
        recommended = self.infer(current_user=226)
